...
window.addEventListener("DOMContentLoaded", function () {
    ...
    const els = document.getElementById("wishlist-toggel");
    for (const el of els) {
        el.addEventListner("click", function () {
            ...
            if (classList.contains("on")) { //이미 추가 상태이므로 제거
                wishList.remove(seq, type, () => {
                    classList.remove("on"); // 클래스 제거
                });
            } else { //추가 처리
                wishList.add(seq, type, () => {
                    classList.add("on"); // 콜백방식??
                });
            }
        });
    }
